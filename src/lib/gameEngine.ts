...
// Re-export CROP_INFO for backward compatibility
export { CROP_INFO };
...
export const TASK_DURATIONS: Record<TaskType, number> = {
  ...
  catch_fish: 2000, // 2 seconds to attempt catching a fish
};
...
function generateFarmerAutoTasks(state: GameState, zone: Zone): Task[] {
  ...
  if (basket.length >= basketCapacity) {
    ...
    console.log('🔍 Priority 1 - Basket full, checking where to go:');
    console.log('  hasTransportBots:', hasTransportBots);
    console.log('  hasMarkedItems:', hasMarkedItems, '(', state.markedForSale.length, 'items)');
    console.log('  autoSell:', farmerAuto.autoSell);
    ...
    console.log('  shouldSell:', shouldSell);
  ...
  for (const hunter of zone.hunterBots) {
    ...
    if (hunter.status === 'patrolling' || hunter.status === 'idle') {
      ...
      if (!targetRabbit || targetRabbit.status === 'captured') {
        ...
        if (dist < 0.5) {
          // Caught the rabbit! Clear any eating state
          updatedRabbits = updatedRabbits.map(r =>
            r.id === targetRabbit.id ? {
              ...r,
